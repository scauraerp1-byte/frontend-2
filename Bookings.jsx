...
export default function Bookings() {
  ...
  // "pending"    = confirmed & not yet dispatched (default)
  // "cancelled"  = status=cancelled
  // "after"      = confirmed & dispatched (already moved to Dispatch)
  ...
  const load = () => {
    ...
    if (view === "cancelled") {
      ...
      params.include_dispatched = true;
    ...
    api.get("/bookings", { params }).then((r) => {
      let rows = r.data;
      if (view === "after") rows = rows.filter((b) => b.dispatched);
      else if (view === "pending") rows = rows.filter((b) => !b.dispatched);
      setItems(rows);
    });
  };
        ...
        filters={[
          {
            ...
            options: [
              ...
              { value: "after", label: "After Dispatch" },
            ],
          },
        ]}
      />
