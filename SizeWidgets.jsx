...
/**
 * Auto-initialise sizes for a product when it's added to a booking / estimate /
 * dispatch / vendor-return / etc. Wholesale ERPs default every available size
 * with quantity = 1 so the user only needs to adjust upward or set to 0.
 *
 * @param {string} preset  e.g. "M-XXXL"
 * @param {number} qty     default quantity per size (default 1)
 * @param {object} [max]   optional stock-by-size cap; qty is clamped ≤ cap
 * @returns {Record<string, number>}
 */
export function initSizesForPreset(preset, qty = 1, max) {
  const sizes = PRESETS[preset] || [];
  const entries = sizes.map((s) => {
    if (max && Object.prototype.hasOwnProperty.call(max, s)) {
      return [s, Math.max(0, Math.min(qty, Number(max[s]) || 0))];
    }
    return [s, qty];
  });
