...
import { ScanLine, X, Loader2, Plus, CheckCircle2 } from "lucide-react";
...
export default function DispatchForm() {
  ...
  const addProductFull = (p) => {
    ...
    // Auto-populate every size with qty = 1, capped by available stock.
    ...
    <div className="max-w-3xl mx-auto space-y-5">
      ...
      <GlassCard>
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => setPickerOpen(true)} data-testid="dispatch-pick-open" className="rounded-full glass px-4 py-3 text-xs uppercase tracking-[0.2em] hover:bg-white/10 inline-flex items-center gap-2">
            <Plus className="w-4 h-4 text-[#ebd281]" /> Pick
          </button>
      ...
      <ProductPicker
        ...
        onPick={addProductFull}
        onScan={() => { setPickerOpen(false); setScanOpen(true); }}
        title="Add product to dispatch"
      />
