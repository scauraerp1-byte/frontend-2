...
import { Plus, X, Loader2, ScanLine } from "lucide-react";
...
export default function BookingForm({ editMode = false }) {
  ...
  const addProduct = (p) => {
    ...
    // Auto-populate every available size with qty = 1 (wholesale default).
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-5">
      ...
      <ProductPicker
        ...
        onPick={addProduct}
        onScan={() => { setProductPicker(false); setScanOpen(true); }}
        title="Add product to booking"
      />
    </div>
  );
}
